...
function checksCreateTodosUserAvailability(request, response, next) {
    ...
    if (user.pro === false && user.todos.length > 9)
        return response.status(403)
            .json({ error: 'You already have 10 todos, sorry it is not possible to create more todos. Upgrade to the PRO plan and create unlimited todos!' })
...
function findUserById(request, response, next) {
    ...
    // Verify if exists Account by ID
